...
for ARN in $CONTAINER_INSTANCE_ARNS; do
    ...
    CONTAINER_IDS=$(ssh -i qrscanner.pem ec2-user@$PUBLIC_DNS "docker ps --filter \"ancestor=sanjayvrs/mediafinalred:2\" --format '{{.ID}}'")
    ...
    for CONTAINER_ID in $CONTAINER_IDS; do
      ...
      # INSTALL_COMMAND="php maintenance/install.php --dbname=new --dbserver=\"properdb-masterdb-gy7idc4sjgqr.cvu2eyysedhj.us-east-2.rds.amazonaws.com\" --installdbuser=root --installdbpass=october2023 --dbuser=root --dbpass=october2023 --server=\"$SERVER_URL\" --scriptpath=\"\" --lang=en --pass=Adminpassword \"Wiki Name\" \"Admin\""
        ...
        ssh -i qrscanner.pem ec2-user@$PUBLIC_DNS "docker exec $CONTAINER_ID php maintenance/install.php --dbname=$DB_NAME --dbserver="$DB_SERVER" --installdbuser=root --installdbpass=october2023 --dbuser=root --dbpass=october2023 --server="$SERVER_URL" --scriptpath="" --lang=en --pass=Adminpassword "Wiki Name" "Admin""
    done
done
